...
function readFile() {
    d3.loadData('annotations.json', 'matches.tsv', function (err, res) {
        ...
        matches.forEach(function (d, i) {
            ...
            if (d.date < "10-20") teams2wins[d.wName]++     //make sure to change key each year
        })
...
function scoreMatches(matches) {
    ...
    d3.nestBy(teams, function (d) {
        return d.w + d.wins * 10
    })
            ...
            if (d.length <= advanceSlots) {
                ...
                if (advanceSlots == 2) {
                    if (d.length == 2) { //check for tied first place
                        against = matches.filter(match => {
                            return match.allTeams == d[0].name + '-' + d[1].name
                                || match.allTeams == d[1].name + '-' + d[0].name
                        })
                        if (against[0].actualWinner == against[1].actualWinner) {
                            d.forEach(function (d) {
                                if (d.name == against[0].actualWinner) d.advance = 'u'
                            })
                        }
                    } else
                        d.forEach(function (d) {
                ...
                if (d.length == 2) { //check for tied last place
                    ...
                    if (against[0].actualWinner == against[1].actualWinner) {
                        d.forEach(function (d) {
                            if (d.name != against[0].actualWinner) d.advance = 'e'
                        })
                    }
                }
                if (advanceSlots == -1) {
...
function drawGroup(gMatches) {
    ...
    var incomplete = gMatches.filter(function (d) {
        return !d.complete
    })
    ...
    var gameSel = sel.append('div.matches')
        ...
        .on('click', function (d) {
            ...
            d3.select(this).selectAll('.teamabv')
                .classed('won', function (e, i) {
                    return i + 1 == d.clicked
                })
...
function drawResults(sel, scenarios, name, complete, incomplete) {
    ...
    scenarios.forEach(function (d) {
        d.team = d.teams.filter(function (d) {
            return d.name == name
        })[0]
        d.wins = d.team.wins
        ...
        d.recordStr = d.playedIn.map(function (d) {
            return +d.currentWon
        }).join('')
    })
    ...
    var pLost = completeIn.filter(function (d) {
        return !d.currentWon
    })
    ...
    var winsSel = svg.appendMany('g', byWins.sort(d3.descendingKey('key')))
        .translate(function (d, i) {
            return [0, i * 80 + (i == 3 ? -15 * 2 : i > 0 ? -8 : 0)]
        })
    ...
    var recordSel = winsSel.appendMany('g', ƒ('byRecordStr'))
        .translate(function (d, i) {
            return [d.key == '000' || d.key == '111' ? 100 : i * 100, 0]
        })
    ...
    recordSel.appendMany('circle.scenario', ƒ())
        .at({
            r: 5, fill: ƒ('team', color[colorChoice]), cx: function (d, i) {
                return i * 10
            }
        })
        .call(d3.attachTooltip)
        .on('mouseout', function () {
            gSel.selectAll('circle.scenario').classed('active', false).at('r', 5)
        })
        .on('mouseover', function (d) {
            ...
            gameSel.append('span').text(ƒ('t1')).classed('won', function (e, i) {
                return d.str[i] == 1
            })
            gameSel.append('span').text(' v. ')
            gameSel.append('span').text(ƒ('t2')).classed('won', function (e, i) {
                return d.str[i] == 2
            })
            ...
            var byAdvanceSel = tt.appendMany('div.advance', d3.nestBy(d.teams, ƒ('advance')).sort(d3.descendingKey('key')))
                .text(function (d) {
                    return d.map(ƒ('name')).join(' and ') + {
                        ...
                        t: ' advance',
                        m: ' tie',
                        f: (d.length > 1 ? ' are' : ' is') + ' eliminated',
                        e: ' last place'
                    }[d.key]
                })
        })
...
color = [function color(d) {
    return {u: '#4CAF50', t: '#4CAF50', m: '#FF9800', f: '#f73434', e: '#f73434'}[d.advance]
},
    function color(d) {
        return {u: '#2c7bb6', t: '#2c7bb6', m: '#dfaf90', f: '#f73434', e: '#f73434'}[d.advance]
    },
    function color(d) {
        return {u: '#36e03d', t: '#4CAF50', m: '#FF9800', f: '#f73434', e: '#b80f02'}[d.advance]
    },
    function color(d) {
        return {u: '#5aa4db', t: '#2c7bb6', m: '#dfaf90', f: '#f73434', e: '#b80f02'}[d.advance]
    }]
